...
const stagedTs = staged.filter(
  (f) => f.endsWith(".ts") || f.endsWith(".tsx") || f.endsWith(".mts"),
);
...
if (stagedTs.length > 0) {
  ...
  if (existsSync(madgeBin)) {
    const result = spawnSync(
      madgeBin,
      ["--circular", "--extensions", "ts,tsx", "src"],
      { encoding: "utf8" },
    );
...
if (stagedTs.length > 0) {
  ...
  for (const file of stagedTs) {
    ...
    lines.forEach((line, i) => {
      // Match the markers only in a comment context, not when they appear inside code or a regex.
  ...
  if (hits.length > 0) {
    warn(
      `TODO/FIXME found in staged files`,
      hits.slice(0, 3).join(", ") +
...
const guidesJsonStaged = stagedJson.find(
  (f) => f === "guides.json" || f.endsWith("/guides.json"),
);
if (guidesJsonStaged) {
  try {
...
if (stagedTs.length > 0) {
  ...
  if (hits.length > 0) {
    warn(
      "console.log found in staged TS files",
      hits.slice(0, 3).join(", ") +
        (hits.length > 3 ? ` (+${hits.length - 3} more)` : ""),
    );
  } else {
