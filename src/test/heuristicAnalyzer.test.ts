...
} from "../analyzer/heuristicAnalyzer.js";
...
describe("analyzeHeuristic — keyword signals", () => {
  it("assigns high tier to architecture prompts", () => {
    ...
    const result = analyzeHeuristic(
      meta("design the architecture for a microservices system", {
        estimatedPromptTokens: 200,
      }),
    );
    const archSignal = result.signals.find(
      (s) => s.label === "Architecture task",
    );
  ...
  it("assigns high tier to race condition / concurrency prompts", () => {
    ...
    const result = analyzeHeuristic(
      meta("why does this race condition cause a deadlock in the thread pool", {
        estimatedPromptTokens: 200,
      }),
    );
  ...
  it("assigns low-medium tier to refactoring prompts", () => {
    ...
    const result = analyzeHeuristic(
      meta("refactor this module to improve performance", {
        estimatedPromptTokens: 200,
      }),
    );
    const refactorSignal = result.signals.find(
      (s) => s.label === "Refactoring task",
    );
...
describe("analyzeHeuristic — length signals", () => {
  it("adds positive weight for long prompts (>800 tokens)", () => {
    ...
    const result = analyzeHeuristic(
      meta(longPrompt, { estimatedPromptTokens: 900 }),
    );
    const lengthSignal = result.signals.find(
      (s) => s.type === "length" && s.weight > 0,
    );
...
describe("analyzeHeuristic — context signals", () => {
  it("increases score with large context file count", () => {
    const withContext = analyzeHeuristic(
      meta("fix the bug", { contextFileCount: 6, contextFileTokens: 10000 }),
    );
  ...
  it("increases score with large code selection", () => {
    ...
    const selectionSignal = result.signals.find((s) =>
...
describe("analyzeHeuristic — tier/effort mapping", () => {
  it("maps score 1–2 to minimal tier", () => {
    const result = analyzeHeuristic(
      meta("what is JSON", { estimatedPromptTokens: 10 }),
    );
    if (result.score <= 2) {
...
describe("effortToThinkingBudget", () => {
  it("returns 0 for none", () =>
    expect(effortToThinkingBudget("none")).toBe(0));
  it("returns 1024 for low", () =>
    expect(effortToThinkingBudget("low")).toBe(1024));
  it("returns 4096 for medium", () =>
    expect(effortToThinkingBudget("medium")).toBe(4096));
  it("returns 10000 for high", () =>
    expect(effortToThinkingBudget("high")).toBe(10000));
  it("returns 32000 for max", () =>
    expect(effortToThinkingBudget("max")).toBe(32000));
});
